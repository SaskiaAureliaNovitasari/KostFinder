...
<section class="hero">
 ...
 <div class="search-box">
  <span>📍</span>
  <input type="text" id="inputCari" placeholder="Cari kost di Jabodetabek"/>
  <button class="search-btn" onclick="cariKost()">Cari Kost →</button>
</div>
...
<section id="kost">
  <div class="section-inner">
    <div class="reveal fav-header">
      ...
      <p id="filter-label" style="color:var(--muted);font-size:14px;align-self:flex-end;"></p>
    </div>
...
<div style="text-align: center; margin-top: 40px;">
  <button class="btn-primary" onclick="lihatLebihBanyak()">Lihat Kost Lebih Banyak</button>
</div>

<div class="kost-grid" id="kostTerkunci" style="display: none; margin-top: 24px;"></div>
  </div>
</section>

<section id="top-favorit" style="background: var(--navy);">
  <div class="section-inner">
    <div class="reveal">
      <div class="section-label">Paling Populer</div>
      <h2 class="section-title">Kost <span style="color:var(--coral)">Top Favorit</span></h2>
      <p class="section-sub">Pilihan kost paling banyak disukai oleh pengguna.</p>
    </div>

    <div class="kost-grid reveal" id="top-favorit-grid">
      <!-- isi by JS -->
    </div>
  </div>
</section>
...
function cariKost() {
  ...
  let hasil = kostData.filter(k =>
    k.name.toLowerCase().includes(keyword) ||
    k.loc.toLowerCase().includes(keyword)
  );
...
// ─── DATA ───
const kostData = [
  { id: 1, name: 'Kost Melati Putih', loc: 'Jember Kota, Jember', price: 450000, tier: 'ekonomis', img: 'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=400&q=80', rating: 4.2, reviews: 24, facs: ['WiFi', 'Air Panas', 'Parkir'], badge: 'Tersedia' },
  { id: 2, name: 'Kost Griya Asri', loc: 'Sumbersari, Jember', price: 750000, tier: 'standar', img: 'https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=400&q=80', rating: 4.6, reviews: 38, facs: ['WiFi', 'AC', 'KM Dalam', 'Laundry'], badge: 'Populer', badgeTeal: true },
  { id: 3, name: 'Kost Residence 88', loc: 'Patrang, Jember', price: 1500000, tier: 'premium', img: 'https://images.unsplash.com/photo-1598928506311-c55ded91a20c?w=400&q=80', rating: 4.9, reviews: 61, facs: ['WiFi', 'AC', 'TV', 'Gym', 'Kulkas'], badge: 'Premium' },
  { id: 4, name: 'Kost Barokah', loc: 'Kaliwates, Jember', price: 380000, tier: 'ekonomis',  img: 'https://images.unsplash.com/photo-1523217582562-09d0def993a6?w=400&q=80' , rating: 4.0, reviews: 15, facs: ['WiFi', 'Parkir', 'Keamanan'], badge: 'Tersedia' },
  { id: 5, name: 'Kost Bintang Mas', loc: 'Tegalboto, Jember', price: 950000, tier: 'standar', img: 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=400&q=80', rating: 4.5, reviews: 47, facs: ['WiFi', 'AC', 'Dapur', 'KM Dalam'], badge: 'Baru', badgeTeal: true },
  { id: 6, name: 'Kost Villa Elok', loc: 'Mangli, Jember', price: 2200000, tier: 'premium', img: 'https://images.unsplash.com/photo-1505691938895-1758d7feb511?w=400&q=80', rating: 5.0, reviews: 29, facs: ['WiFi', 'AC', 'Kolam Renang', 'Parkir Mobil', 'Cleaning'], badge: 'Eksklusif' },
];
    ...
    <div class="kost-card reveal" id="kost-${k.id}">
      <div class="kost-img">
        <img 
          ...
          onerror="this.onerror=null;this.src='https://via.placeholder.com/400x250?text=Kost';"
        >
        <span class="kost-badge ${k.badgeTeal ? 'teal' : ''}">${k.badge}</span>

        <button class="fav-btn ${favorites.includes(k.id) ? 'active' : ''}" 
      ...
      <div class="kost-body">
        ...
        <div class="kost-facs">
          ${k.facs.map(f => `<span class="tag">${f}</span>`).join('')}
        </div>
      </div>
      ...
      <div class="kost-footer">
        <div class="rating">
          <span class="stars">
            ${'★'.repeat(Math.floor(k.rating))}${'☆'.repeat(5 - Math.floor(k.rating))}
          </span>
...
function renderFav() {
    ...
    <div class="kost-grid" id="fav-grid">
      ${data.map(k => `
        <div class="kost-card reveal">
          <div class="kost-img">
            <img 
              src="${k.img || 'https://via.placeholder.com/400x250?text=Kost'}"
              style="width:100%;height:100%;object-fit:cover;"
              onerror="this.onerror=null;this.src='https://via.placeholder.com/400x250?text=Kost';"
            >

            <span class="kost-badge ${k.badgeTeal ? 'teal' : ''}">
              ${k.badge || 'Kost'}
            </span>
          ...
          <div class="kost-body">
            ...
            <div class="kost-price">
              Rp ${(k.price || 0).toLocaleString('id-ID')}<span>/bulan</span>
            </div>
          ...
          <div class="kost-footer">
            <div class="rating">
              <span class="stars">
                ${'★'.repeat(Math.floor(k.rating || 0))}${'☆'.repeat(5 - Math.floor(k.rating || 0))}
              </span>
              <strong>${k.rating || 0}</strong>
              <span class="count">(${k.reviews || 0})</span>
            </div>
            <button class="detail-btn">Lihat Detail</button>
          </div>
        </div>
  ...
  function renderTopFavorit() {
  const top = kostData.slice(0, 3);
  const grid = document.getElementById('top-favorit-grid');

  if (!grid) return;

  grid.innerHTML = top.map(k => `
    <div class="kost-card">
      <div class="kost-img">
        <img 
          src="${k.img}" 
          ...
          onerror="this.onerror=null;this.src='https://via.placeholder.com/400x250?text=Kost';"
        >
        <span class="kost-badge">${k.badge}</span>
      </div>
      <div class="kost-body">
        <div class="kost-name">${k.name}</div>
        <div class="kost-loc">📍 ${k.loc}</div>
...
renderTopFavorit();
  ...
  function lihatLebihBanyak() {
    const login = localStorage.getItem('kf-login');
    ...
    const container = document.getElementById('kostTerkunci');
    container.style.display = 'grid';

    container.innerHTML = dataKostTerkunci.map(k => `
      <div class="kost-card">
        <div class="kost-img">
          ${k.emoji}
          <span class="kost-badge">${k.badge}</span>
...
">
  ...
  ">
    ...
    <p style="font-size:14px; color:#555; margin-bottom:20px;">
      Jika ingin melihat kost lebih banyak,<br>
      silahkan login terlebih dahulu.
    </p>

    <!-- BUTTON -->
    <div style="display:flex; gap:10px; justify-content:center;">
      <button class="btn-primary" onclick="goLogin()">Login Sekarang</button>
      <button class="btn-outline" onclick="closePopup()">Tutup</button>
    </div>
  </div>
</div>
